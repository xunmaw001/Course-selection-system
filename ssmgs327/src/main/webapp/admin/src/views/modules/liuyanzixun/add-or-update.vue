...
export default {
  ...
  methods: {
	...
	this.ruleForm.zixunwenti = this.ruleForm.zixunwenti.replace(reg,'../../../ssmgs327/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `liuyanzixun/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.liuyanzixunCrossAddOrUpdateFlag = false;
						 this.parent.search();
